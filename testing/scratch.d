...
+/
	...
	/+
		...
		{                                  #
			...
			{                              # (
				Node->Identifier           #   Bar
				Node^Identifier->Children  #   Baz
				{                          #   (
					Node->Identifier       #     qux
					Node->Identifier       #     quux
				}                          #   )
				Node->Identifier           #   Qux
				Node->Identifier           #   Quux
			}                              # )
			Node^Identifier->Children      # BAR
			{                              # (
				Node->Identifier           #   Foo
			}                              # )
		}                                  #
	...
	stdout.writefln("%s", TreeGrammar.decimateTree(t2));
	
	auto t3 = TreeGrammar.Root(
		"
		Root->Children                      //
		{                                   //
			Node->/Identifier->Children     // FOO
		...
		");
	stdout.writefln("%s", TreeGrammar.decimateTree(t3));
	+/
	
	/*
	auto t4 = TestGrammar.Root("a.");
	stdout.writefln("%s", t4);
	*/
	/+"
		Foo
		-Bar
...
void lowerWhileStatement( SyntaxElement* syntaxNode )
{
    ...
    syntaxNode.replaceWith( captures, 
        ...
        {
            OpNegate->&expr
            GotoStatement->&uniqExitLoop
        }
        $statement
        GotoStatement->&uniqLoopAgain
        Label->&uniqExitLoop
        ")
